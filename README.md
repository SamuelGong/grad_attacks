...
由于上述文献发表时间相对比较早，针对文本领域内的梯度泄露攻击我又再调研了一下， 发现了以下一篇最前沿的工作。
...
3. [LAMP: Extracting Text from Gradients with Language Model Priors (NeurIPS '22)](https://arxiv.org/pdf/2202.08827.pdf)
   - 参考图像领域的经验，该论文提出 LAMP，尝试利用先验知识来提高文本领域的**梯度匹配**攻击效果。
   特别地，LAMP 周期性地会对恢复的中间结果（虚拟特征）进行一个额外的优化。
   ...
   对每个序列计算一个损失函数值。
   由于语言建模模型的损失函数值有物理意义（困惑度的对数），可以认为损失函数值最小的序列，
   最符合自然语言的习惯（这也就是所谓的先验知识），从而认为是最优。
   上述额外优化是本论文的主要贡献，有新意。
   作者在文本分类任务上和 TAG 进行了对比，结果显示相比 TAG 恢复效果有提升。

**【参考资料】** 虽然前两篇文章皆无直接公开的代码，但认真搜索一番想必不难找到一个名为 [JonasGeiping/breaching](https://github.com/JonasGeiping/breaching) 
...
第三篇文章有公开的代码，在名为 [eth-sri/lamp](https://github.com/eth-sri/lamp) 的 GitHub 仓库。
...
在复现第三篇文章时，由于框架已经大致搭好，最多只有一些超参数的设置是未知数。
于是，虽然有原文仓库，但几乎不需要参考了。
...
1. 迭代 1000 轮（约 269 秒）：token 准确率 0.25，RougeL 0.4286，具体恢复文本如下

```
 The Tower Building of Daryl Hunting OF The Monthly ep bruised wise Building Little Lynating
```

2. 迭代 20000 轮（约 4059 秒）：token 准确率 0.1875，RougeL 0.3846，具体恢复文本如下
...
|  3月5日   | 3月6日至10日 |    3月11日     | 3月12日至13日  |     3月14日至15日     |
|:-------:|:--------:|:------------:|:--------:|:-------------:|
| 读文章、搭框架 |  调试 TAG  | 调通 TAG 并撰写文档 | 调通 APRIL 并撰写文档 | 调通 LAMP 并撰写文档 |
